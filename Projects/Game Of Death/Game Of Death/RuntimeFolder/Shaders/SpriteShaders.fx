...
SVSOut  GUIBoundsVS( SVSIn sVSData )
{
	SVSOut vsOut;
...
{
	return tex2D( g_sTexture0NoFilter, CorrectTexCoordForRes(vTexCoord) );
}
float4 GUIBoundsPS( ) : COLOR0
{
	float4 fReturn;
	fReturn.a = 0.5f;
	fReturn.r = 1.0f;
...
technique GUI_Bounds
{
	pass P0
	{
		VertexShader = compile vs_3_0 GUIBoundsVS();
		PixelShader = compile ps_3_0 GUIBoundsPS();
